       ...
       complete:(QNCompleteBlock)complete
{
    ...
    AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        complete(request, response, nil, JSON);
        ...
        complete:(QNCompleteBlock)complete
{
  //  @autoreleasepool {
    ...
    weakChunkComplete = chunkComplete = ^(NSURLRequest *request, NSHTTPURLResponse *response, NSError *error, id JSON)
    {
        if (error != nil) {

            if (retryTime == 0 || isMkblock || [response statusCode] == 701) {
                complete(request, response, error, JSON);
                return;
            } else {
                retryTime --;
            ...
            isMkblock = NO;
            blockPutRet = [[QiniuBlkputRet alloc] initWithDictionary:JSON];
